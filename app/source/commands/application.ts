...
import { COMPONENTS } from '../functions/discord-simplifiers';
...
import { out, read_json, count, get_time } from '../functions/simplifiers';
import { gen_embed, gen_button, gen_attach } from '../functions/discord-simplifiers';
        ...
        .then(async APP_CHANNEL => {
            ...
            message_events.on('end', async shards => {
                ...
                if(merged_messages.length >= 2000) {
                    ...
                    if(!attachment)
                        throw new Error('Error occured via handling an attachment!');
                ...
                switch(ADMIN_APPLIES_CHANNEL.type) {
                    ...
                    case 0:
                        ...
                        const session_time: number = get_time('time');
