...
export function init() {
    ...
    client.on('messageReactionAdd', async (reaction, user) => {
        ...
        const applications_array: [ { [key: string]: any }] = await read_json('./data/applications.json');
        ...
        if(application) {
             ...
             */
            ...
            else if(reaction.emoji.name === '❎' && application['STATUSES']['IS_ACCEPTED'] == false) {
                const new_applications = applications_array.find(object => object['ADMINS_CHANNEL_MESSAGE_ID'] !== message_id);
                ...
                .then(async DISWELCOMED_MESSAGE => {
                    ...
                    await reaction.message.guild?.channels.fetch(application['TICKET_CHANNEL_ID'])
